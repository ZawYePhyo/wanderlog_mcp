...
export class WanderlogAuthError extends WanderlogError {
  constructor(message = "Wanderlog session invalid or expired") {
    super(message, "auth_expired", {
      hint: "Run 'wanderlog_mcp-auth' to log in again, then restart your MCP client.",
      followUps: [
        "Ask the user to run 'wanderlog_mcp-auth' in their terminal to refresh the session, then restart the MCP client.",
      ],
    });
