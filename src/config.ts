...
import { readAuthFile } from "./auth-storage.js";
...
export function normalizeCookie(raw: string): string {
  ...
  throw new WanderlogValidationError(
    ...
    "Expected a value starting with 's%3A' or 'connect.sid=s%3A'. Run 'wanderlog_mcp-auth' instead of setting this manually.",
  );
}

export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config {
  // Priority: env var → saved auth file (~/.wanderlog_mcp/auth.json)
  let raw = env.WANDERLOG_COOKIE ?? readAuthFile()?.cookie;

  if (!raw) {
    throw new WanderlogValidationError(
      "Not authenticated — run 'wanderlog_mcp-auth' to log in",
      "Run: npx wanderlog_mcp-auth\n\nThis opens a browser window. Log in to Wanderlog once and the session is saved automatically. No env var needed.",
    );
  }
