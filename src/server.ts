...
} from "./tools/delete-expense.js";
import {
  getBudgetSummary,
  getBudgetSummaryDescription,
  getBudgetSummaryInputSchema,
} from "./tools/get-budget-summary.js";
import {
  listExpenses,
...
export function buildServer(ctx: AppContext): McpServer {
  ...
  server.registerTool(
    ...
    async (args) => listExpenses(ctx, args as Parameters<typeof listExpenses>[1]),
  );

  server.registerTool(
    "wanderlog_get_budget_summary",
    {
      title: "Get budget summary for a Wanderlog trip",
      description: getBudgetSummaryDescription,
      inputSchema: getBudgetSummaryInputSchema,
    },
    async (args) => getBudgetSummary(ctx, args as Parameters<typeof getBudgetSummary>[1]),
  );

  server.registerTool(
    "wanderlog_add_expense",
    {
      title: "Add an expense to a Wanderlog trip",
      description: addExpenseDescription,
      inputSchema: addExpenseInputSchema,
    },
    async (args) => addExpense(ctx, args as Parameters<typeof addExpense>[1]),
  );

  server.registerTool(
    "wanderlog_delete_expense",
    {
      title: "Delete an expense from a Wanderlog trip",
      description: deleteExpenseDescription,
      inputSchema: deleteExpenseInputSchema,
    },
    async (args) => deleteExpense(ctx, args as Parameters<typeof deleteExpense>[1]),
  );
